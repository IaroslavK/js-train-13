...
function searchParamsURL(url) {
    ...
    const obURLSearchParams = newUrl.searchParams;
    const params = new Map();
    for (const [key, value] of obURLSearchParams) {
...
function checkURLParameters(url, params) {
    ...
    for (const value of params) {
        newObj[value] = urlObj.searchParams.has(value);
    }
    return newObj;
    // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function sortUrlParams(url) {
    ...
    arr.sort();
    urlObj.searchParams.delete(urlObj.search);
    urlObj.search = "";
...
function getURLValues(url) {
    ...
    const arr = [];
    for (const key of keys) {
        const keyValue = urlObj.searchParams.getAll(key);
        arr.push(...keyValue);
    }
